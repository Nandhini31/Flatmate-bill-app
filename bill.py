...
class PDFReport:
    ...
    def generate(self, flatmate1, flatmate2, bill):
        ...
        pdf.cell(w=100, h=40, txt=f"{bill.period}", border=1, ln=1)
        pdf.cell(w=200, h=40, txt="Total amount:", border=1,)
        pdf.cell(w=100, h=40, txt=f"{bill.amount}", border=1, ln=1)
        pdf.cell(w=100, h=40, txt=f"{flatmate1.name}:", border=1,)
        pdf.cell(w=100, h=40, txt=f"£{flatmate1_amount}", border=1, ln=1)
        pdf.cell(w=100, h=40, txt=f"{flatmate2.name}:", border=1, )
        pdf.cell(w=100, h=40, txt=f"£{flatmate2_amount}", border=1, ln=1)
